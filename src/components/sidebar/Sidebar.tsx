      ...
      <nav className="flex-1 mt-10 px-6 space-y-6">
        {navItems.map((item) => (
          <NavLink
            ...
            to={item.path}
            className={({ isActive }) =>
              `flex items-center gap-4 px-4 py-2 transition-all duration-200 group ${isActive ? 'text-[#114232] font-bold' : 'text-[#114232] font-semibold opacity-80 hover:opacity-100'
              }`
            }
          >
            <item.icon
